...
contract mktBank {
    ...
    constructor(address _mkt, address _hutbank, uint8 _act) {
        require(_mkt != address(0), "mkt zero");
        ...
        )
    {
        ...
        // 실제 보유(지갑 + 스테이킹)
        myActualQty = mkt.balanceOf(who) + u.depo;
    ...
    function buy(uint256 amount, uint256 maxPay) external returns (bool) {
        ...
        if (userReceive > 0) {
            require(mkt.transfer(msg.sender, userReceive), "token transfer fail");
        }
    ...
    function sell(uint256 amount) external returns (bool) {
        ...
        require(mkt.allowance(msg.sender, address(this)) >= amount, "allowance low");
        require(mkt.transferFrom(msg.sender, address(this), amount), "token transfer fail");
    ...
    function withdraw() external returns (bool) {
        ...
        require(mkt.transfer(msg.sender, amount), "token transfer fail");
